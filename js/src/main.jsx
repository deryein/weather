...
class SelectWeatherBtnSet extends React.Component{
	...
	render(){
		return(
			...
			<div>
            	<button data-city = "taipei" type = "button" className = 'btn btn-success' onClick={this.clickCity}>Taipei</button>
            	<button data-city = "kaohsiung" type = "button" className = 'btn btn-primary' onClick={this.clickCity}>Kaohsiung</button>
        		<div>地區：{this.state.area}</div>
            	<div>溫度：{this.state.temp}</div>
        	</div>
...
ReactDOM.render(<SelectWeatherBtnSet />, document.getElementById('btn-set'));
